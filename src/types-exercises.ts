...
function testExercise1() {
  ...
  console.log([
    'hero as OmitBoolean',
    Object.getOwnPropertyNames(hero as OmitBoolean<Hero>),
  ]) /// [ 'hero as OmitBolean', [ 'id', 'name', 'description', 'email', 'isAlive' ] ]
  ...
  const hero3: OmitBoolean<Hero> = hero
  console.log(hero3)
}
testExercise1()
...
const number4: If<false, string, number> = 4
const text4: If<true, string, number> = '4'
...
console.log([typeof number4, typeof text4]) // ['number','string']
...
// Add here your example
const heroReadonly: MyReadonly<Hero> = hero
...
function testExercise4() {
  ...
  const result: MyReturnType<typeof fn> = fn(true)
...
type MyAwaited<T> = T extends {
  then(onfulfilled: (value: infer R) => any): any
}
  ? R
  : T
...
//Nesting previous utility type MyReturnType and a promise from a function
const getHeroAsync = (): Promise<Hero> => {
  return new Promise((resolve) => resolve(hero))
}
type heroFn = MyAwaited<MyReturnType<typeof getHeroAsync>>
const hero4: heroFn = {
  ...hero,
}
